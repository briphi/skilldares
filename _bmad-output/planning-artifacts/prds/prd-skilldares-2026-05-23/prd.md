...
- **FR11.1** After an MC option is selected, the question SHALL transition through a two-stage reveal sequence before the feedback overlay appears. Total reveal duration is asymmetric on correctness:
  - **Correct answer:** ~1500ms total (lock + reveal). The player already knows they got it right; minimal pause.
  - **Wrong answer:** ~3000ms total (lock + extended reveal). The player needs extra time to read the highlighted correct answer they missed.
  - **Lock phase (~400ms, same for both):** the tapped quadrant is visually highlighted; the other three remain at default appearance.
  - **Reveal phase (~1100ms for correct, ~2600ms for wrong):** the correct answer is highlighted via outline + brightness contrast (full color, thicker success-color outline, slight scale-up, subtle glow) — NOT a center overlay badge, which would obscure the option text; all three wrong options dim equally, including the user's pick if it was wrong. The player's wrong pick is NOT given a separate red/✗ treatment — the correct answer is the single visual focus.
